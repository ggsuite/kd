...
import 'package:kd/src/commands/kidney_bash.dart';
...
void main() {
  ...
  setUp(() async {
    ...
    kidneyBash = KidneyBash(ggLog: ggLog);
    arguments = [dRoot.path, '--apply', '--verbose', 'dart', '--help'];
    runner = GgCommandRunner(ggLog: ggLog, command: kidneyBash);
  });
  ...
  group('KidneyBash', () {
    group('run(args)', () {
      group('- main case', () {
        void checkMessages({required bool verbose, required bool apply}) {
          ...
          if (!apply) {
            expect(messages[i++], kidneyBash.missingApplyHelp);
          }

          if (apply && !verbose) {
            expect(messages[i++], kidneyBash.missingVerboseHelp);
          }
        ...
        group(
          ...
          () {
            test('with --apply and --verbose', () async {
              await runner.run(args: [dRoot.path, '-av', 'dart', '--help']);
              checkMessages(apply: true, verbose: true);
            });

            test('with --apply only', () async {
              await runner.run(args: [dRoot.path, '-a', 'dart', '--help']);
              checkMessages(apply: true, verbose: false);
            });

            test('with --verbose only', () async {
              await runner.run(args: [dRoot.path, '-v', 'dart', '--help']);
      ...
      group('- edge cases', () {
        ...
        test('- should throw, if no packages are found', () async {
          ...
          await runner.run(args: [dRoot.path, '-av', 'dart', '--help']);
          expect(messages[0], contains('No dart packages found'));
        });
      });
    });
    ...
    group('readArgs(args)', () {
      group('- main case', () {
        test('- reads kidneyBash args, command args, '
            'directory, verbose and apply', () async {
          final (directory, kidneyArgs, commandArgs, verbose, apply) =
              await kidneyBash.readArgs(arguments);
        ...
        group('- throws an exception with help', () {
          test('- when the list of command arguments is empty', () async {
            ...
            expect(exception, kidneyBash.commandArgumentsMissingHelp);
          });

          test('- when there is an unknown kidneyBash argument', () async {
            late String exception;
            try {
      ...
      group('- edge cases', () {
        group('- interpretes the first argument', () {
          test('- as a folder, when it is a folder', () async {
            final (directory, _, _, _, _) = await kidneyBash.readArgs(
              arguments,
            );
            expect(directory.path, dRoot.path);
          });
          test('- as part of the command, if it is not a folder', () async {
            final (_, _, commandArgs, _, _) = await kidneyBash.readArgs([
              'x',
              'y',
            ]);
        ...
        group('- removes the first argument, when it ends with', () {
          test('- kidney', () async {
            ...
            final (_, kidneyArgs, _, _, _) = await kidneyBash.readArgs(
              arguments,
            );
            expect(kidneyArgs, ['--apply', '--verbose']);
          });
        ...
        group('- removes the second argument, when it is', () {
          test('- kidney bash', () async {
            arguments = ['kidney', 'bash', ...arguments];
        ...
        group('- interpreates - a as --apply and -v as --verbose', () {
          test('- when they are the first arguments', () async {
            final (_, kidneyArgs, _, verbose, apply) = await kidneyBash
                .readArgs(['-a', '-v', 'x', 'y']);
            expect(kidneyArgs, ['--apply', '--verbose']);
            expect(verbose, true);
            expect(apply, true);
